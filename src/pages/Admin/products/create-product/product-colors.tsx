...
const ProductColors = ({ infos, setInfos }: ProductColorsProps) => {
    const { colors } = infos

    function addColor(color: string) {
        if (infos.colors.includes(color)) return toast.error('Color already exists')
        setInfos((preInfos) => ({
            ...preInfos,
            colors: [...preInfos.colors, color],
        }))
    }
    function removeColor(color: string) {
        setInfos((preInfos) => ({
            ...preInfos,
            colors: preInfos.colors.filter((c) => c !== color),
        }))
    }
